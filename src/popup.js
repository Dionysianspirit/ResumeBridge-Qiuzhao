...
const els = {
  ...
  variantFeedback: document.getElementById("variantFeedback"),
...
let variantLibrary = null;
let variantChanging = false;
let filling = false;
let catalogContext = null;
document.getElementById("openCatalog").addEventListener("click", () => chrome.tabs.create({ url: chrome.runtime.getURL("src/catalog.html") }));
document.getElementById("clearCatalogContext").addEventListener("click", () => {
  catalogContext = null;
  document.getElementById("catalogContext").hidden = true;
});
els.startAutofillBtn.disabled = true;
els.showProfilePanelBtn.disabled = true;
els.profileVariantSelect.addEventListener("change", () => void switchPopupVariant());
...
async function initialize() {
  ...
  await Promise.allSettled([syncRuntimeState(), loadCurrentJobPageInfo(), loadPopupVariants()]);
...
async function loadCatalogContext() {
  try {
    ...
    const result = await sendRuntimeMessage({ type: "OJAF_GET_CATALOG_CONTEXT", payload: { tabId: tab.id, pageUrl: tab.url } });
    catalogContext = result.entry;
    if (!catalogContext) return;
    document.getElementById("catalogContext").hidden = false;
    document.getElementById("catalogContextLabel").textContent = `关联总表：${catalogContext.companyName} · ${catalogContext.batch}（已记录 ${result.submittedCount} 个岗位）。请核对后保存。`;
    els.captureCompanyName.value = catalogContext.companyName;
    if (!els.captureJobTitle.value && catalogContext.targetRole) els.captureJobTitle.value = catalogContext.targetRole;
  } catch (error) {
    setCaptureFeedback(`清单关联读取失败：${error.message}`, "error");
  }
}

function updateVariantControls() {
  const blocked = !variantLibrary || variantChanging || filling;
  els.startAutofillBtn.disabled = blocked;
  els.showProfilePanelBtn.disabled = blocked;
  els.profileVariantSelect.disabled = blocked;
}

function renderPopupVariants(library) {
  ...
  els.profileVariantSelect.value = library.activeId;
  els.variantFeedback.textContent = "在设置中编辑资料，或另存为不同岗位的简历。";
  updateVariantControls();
}

async function loadPopupVariants() {
  try {
    renderPopupVariants(await sendRuntimeMessage({ type: "OJAF_GET_PROFILE_LIBRARY" }));
  } catch (error) {
    variantLibrary = null;
    els.variantFeedback.textContent = `简历加载失败：${error.message}`;
    updateVariantControls();
  }
}

async function switchPopupVariant() {
  const id = els.profileVariantSelect.value;
  variantChanging = true;
  updateVariantControls();
  try {
    renderPopupVariants(await sendRuntimeMessage({
      type: "OJAF_SWITCH_PROFILE",
      payload: { id, expectedActiveProfileId: variantLibrary.activeId, expectedProfileRevision: variantLibrary.revision }
    }));
    els.variantFeedback.textContent = "已切换。当前网页已有内容仍需核对。";
  } catch (error) {
    await loadPopupVariants();
    els.variantFeedback.textContent = error.message;
  } finally {
    variantChanging = false;
    updateVariantControls();
  }
}

async function assertSelectedVariant() {
  const latest = await sendRuntimeMessage({ type: "OJAF_GET_PROFILE_LIBRARY" });
  const changed = latest.activeId !== variantLibrary?.activeId || latest.revision !== variantLibrary?.revision;
  renderPopupVariants(latest);
  if (changed) throw new Error("简历刚在其他窗口更新，请核对当前版本后重新点击。");
}
...
async function saveCurrentApplication() {
  ...
  try {
    const result = await sendRuntimeMessage({
      ...
      payload: {
        application: {
          catalogId: catalogContext?.id || "",
          resumeName: els.profileVariantSelect.selectedOptions[0]?.textContent || "",
