importScripts("safety-policy.js", "message-policy.js", "ai-privacy.js", "job-tracker.js", "catalog-model.js");
...
const MAX_JOB_APPLICATIONS = 10000;
let jobWriteQueue = Promise.resolve();
let catalogPromise;
const CATALOG_TAB_CONTEXT = "catalogTabContext";
...
async function handleMessage(message, sender = {}) {
  ...
  switch (message.type) {
    ...
    case "OJAF_SAVE_SETTINGS":
      return queueProfileWrite(() => saveSettings(message.payload || {}));
    case "OJAF_CLEAR_SETTINGS":
      return queueProfileWrite(clearSettings);
    case "OJAF_GET_PROFILE_LIBRARY":
      await profileWriteQueue;
      return profileLibrarySummary(await readProfileLibrary());
    case "OJAF_CREATE_PROFILE":
      return queueProfileWrite(() => createProfileVariant(message.payload || {}));
    case "OJAF_SWITCH_PROFILE":
      return queueProfileWrite(() => switchProfileVariant(message.payload || {}));
    ...
    case "OJAF_SAVE_JOB_APPLICATION":
      return queueJobWrite(() => saveJobApplication(message.payload || {}));
    case "OJAF_IMPORT_JOB_APPLICATIONS":
      return queueJobWrite(() => importJobApplications(message.payload || {}));
    ...
    case "OJAF_SAVE_CATALOG_NOTE":
      return queueJobWrite(() => saveCatalogNote(message.payload || {}));
    case "OJAF_OPEN_CATALOG_ENTRY":
    ...
    case "OJAF_IMPORT_TRACKING_BACKUP":
      return queueJobWrite(() => importTrackingBackup(message.payload || {}));
    default:
...
async function getSettings(options = {}) {
  ...
  if (options.includeApiConfig !== false) {
    ...
    const library = profileLibraryFromStorage(values);
    settings.activeProfileId = library.activeId;
    settings.profileRevision = library.revision;
    settings.profileLibrary = profileLibrarySummary(library);
  }
  return settings;
}
...
async function saveSettings(payload) {
  ...
  if (payload.profileV2) {
    library = await readProfileLibrary();
    assertProfileContext(library, payload);
    next[STORAGE_KEYS.profileV2] = normalizeProfileV2(payload.profileV2);
    library.entries.find((entry) => entry.id === library.activeId).profileV2 = next[STORAGE_KEYS.profileV2];
    library.revision += 1;
    next[STORAGE_KEYS.profileLibrary] = library;
  }
  ...
  await chrome.storage.local.set(next);
  return { saved: Object.keys(next), ...(library ? { profileRevision: library.revision } : {}) };
}

// Keep profile edits and switches in order, including requests from different tabs.
function queueProfileWrite(action) {
  const result = profileWriteQueue.then(action);
  profileWriteQueue = result.catch(() => undefined);
  return result;
}

function profileLibraryFromStorage(values) {
  const current = normalizeProfileV2(values[STORAGE_KEYS.profileV2] || DEFAULT_PROFILE_V2);
  const stored = values[STORAGE_KEYS.profileLibrary];
  if (!stored) {
    return { activeId: "default", revision: 0, entries: [{ id: "default", name: "通用简历", profileV2: current }] };
  }
  if (!Array.isArray(stored.entries) || stored.entries.length < 1 || stored.entries.length > 20 ||
      !Number.isSafeInteger(stored.revision) || stored.revision < 0 ||
      stored.entries.some((entry) => !entry || typeof entry.id !== "string" || !entry.id ||
        typeof entry.name !== "string" || !entry.name.trim() || !isPlainObject(entry.profileV2)) ||
      new Set(stored.entries.map((entry) => entry.id)).size !== stored.entries.length ||
      !stored.entries.some((entry) => entry.id === stored.activeId)) {
    throw new Error("简历列表格式异常，请保留本地数据并检查备份。");
  }
  return {
    ...
    entries: stored.entries.map((entry) => ({
      id: entry.id,
      name: entry.name,
      profileV2: entry.id === stored.activeId ? current : normalizeProfileV2(entry.profileV2)
    }))
  };
}

async function readProfileLibrary() {
  return profileLibraryFromStorage(await chrome.storage.local.get([
    STORAGE_KEYS.profileV2, STORAGE_KEYS.profileLibrary
  ]));
}

function profileLibrarySummary(library) {
  return {
    activeId: library.activeId,
    revision: library.revision,
    profiles: library.entries.map(({ id, name }) => ({ id, name }))
  };
}

function assertProfileContext(library, payload) {
  if ((payload.expectedActiveProfileId !== undefined && payload.expectedActiveProfileId !== library.activeId) ||
      (payload.expectedProfileRevision !== undefined && payload.expectedProfileRevision !== library.revision)) {
    throw new Error("简历已在其他窗口更新或切换。请先导出未保存的修改，再刷新页面。");
  }
}

async function createProfileVariant(payload) {
  const library = await readProfileLibrary();
  assertProfileContext(library, payload);
  const name = typeof payload.name === "string" ? payload.name.trim() : "";
  if (!name || name.length > 40) throw new Error("请输入 1–40 个字符的简历名称。");
  if (library.entries.some((entry) => entry.name.toLocaleLowerCase() === name.toLocaleLowerCase())) {
    throw new Error("已存在同名简历，请换一个名称。");
  }
  if (library.entries.length >= 20) throw new Error("最多保存 20 份简历。");
  const id = crypto.randomUUID();
  const profileV2 = normalizeProfileV2(payload.profileV2 ||
    library.entries.find((entry) => entry.id === library.activeId).profileV2);
  library.entries.push({ id, name, profileV2 });
  library.activeId = id;
  library.revision += 1;
  await chrome.storage.local.set({
    [STORAGE_KEYS.profileV2]: profileV2,
    [STORAGE_KEYS.profileLibrary]: library
  });
  return profileLibrarySummary(library);
}

async function switchProfileVariant(payload) {
  const library = await readProfileLibrary();
  assertProfileContext(library, payload);
  const target = library.entries.find((entry) => entry.id === payload.id);
  if (!target) throw new Error("这份简历不存在，请刷新简历列表。");
  if (library.activeId !== target.id) {
    library.activeId = target.id;
    library.revision += 1;
    await chrome.storage.local.set({
      [STORAGE_KEYS.profileV2]: target.profileV2,
      [STORAGE_KEYS.profileLibrary]: library
    });
    if (chrome.storage.session?.remove) await chrome.storage.session.remove(PROFILE_PANEL_STATE_KEY);
  }
  return profileLibrarySummary(library);
}

async function clearSettings() {
  const stored = await chrome.storage.local.get([STORAGE_KEYS.profileLibrary]);
  const previousRevision = stored[STORAGE_KEYS.profileLibrary]?.revision;
  const revision = Number.isSafeInteger(previousRevision) ? previousRevision + 1 : 1;
  await chrome.storage.local.set({
    [STORAGE_KEYS.profileV2]: DEFAULT_PROFILE_V2,
    [STORAGE_KEYS.profileLibrary]: { activeId: "default", revision, entries: [{ id: "default", name: "通用简历", profileV2: DEFAULT_PROFILE_V2 }] },
...
function queueJobWrite(action) {
  const result = jobWriteQueue.then(action);
  jobWriteQueue = result.catch(() => undefined);
  return result;
}
...
async function requireCatalogEntry(id) {
  const entry = (await loadCatalog()).entries.find((item) => item.id === id);
  if (!entry) throw new Error("这条招聘信息不存在，请刷新总表。");
  return entry;
}

async function getCatalogState() {
  const stored = await chrome.storage.local.get(["jobApplications", "catalogNotes"]);
  return { applications: normalizeJobApplications(stored.jobApplications), notes: stored.catalogNotes || {} };
}

function normalizeCatalogNote(input) {
  if (!isPlainObject(input)) throw new Error("备注格式不正确。");
  const decision = ["未投递", "准备中", "已放弃"].includes(input.decision) ? input.decision : "未投递";
  const followUpDate = String(input.followUpDate || "");
  if (followUpDate && globalThis.ResumeBridgeCatalog.dateNumber(followUpDate) === null) throw new Error("跟进日期格式不正确。");
  return { decision, targetRole: String(input.targetRole || "").trim().slice(0, 240),
    followUpDate, notes: String(input.notes || "").slice(0, 8000),
    updatedAt: input.updatedAt && Number.isFinite(Date.parse(input.updatedAt))
      ? new Date(input.updatedAt).toISOString() : new Date().toISOString() };
}

async function saveCatalogNote(payload) {
  await requireCatalogEntry(payload.id);
  const stored = await chrome.storage.local.get(["catalogNotes"]);
  const notes = { ...(stored.catalogNotes || {}) };
  if (payload.expectedUpdatedAt !== undefined && payload.expectedUpdatedAt !== (notes[payload.id]?.updatedAt || "")) {
    throw new Error("这条备注已在其他窗口更新，请刷新后重试。");
  }
  notes[payload.id] = normalizeCatalogNote({ ...payload.note, updatedAt: new Date().toISOString() });
  await chrome.storage.local.set({ catalogNotes: notes });
  return { id: payload.id, note: notes[payload.id] };
}

async function openCatalogEntry(payload) {
  const entry = await requireCatalogEntry(payload.id);
  const url = globalThis.ResumeBridgeCatalog.safeWebUrl(entry.applyUrl);
  if (!url) throw new Error("原表投递方式不是单个网页链接，请在详情中查看原文或官方公告。");
  const tab = await chrome.tabs.create({ url });
  const stored = await chrome.storage.session.get(CATALOG_TAB_CONTEXT);
  const contexts = Object.fromEntries(Object.entries(stored[CATALOG_TAB_CONTEXT] || {})
    .filter(([, value]) => Date.now() - value.createdAt < 7200000).slice(-100));
  contexts[tab.id] = { id: entry.id, origin: new URL(url).origin, createdAt: Date.now() };
  await chrome.storage.session.set({ [CATALOG_TAB_CONTEXT]: contexts });
  return { opened: true, tabId: tab.id };
}

async function getCatalogContext(payload) {
  const stored = await chrome.storage.session.get(CATALOG_TAB_CONTEXT);
  const context = stored[CATALOG_TAB_CONTEXT]?.[payload.tabId];
  const url = globalThis.ResumeBridgeCatalog.safeWebUrl(payload.pageUrl);
  if (!context || !url || Date.now() - context.createdAt >= 7200000 || new URL(url).origin !== context.origin) return { entry: null };
  const entry = await requireCatalogEntry(context.id);
  const state = await getCatalogState();
  return { entry: { id: entry.id, companyName: entry.companyName, batch: entry.batch,
    targetRole: state.notes[entry.id]?.targetRole || "", roles: entry.roles },
    submittedCount: state.applications.filter((a) => a.catalogId === entry.id).length };
}

async function exportTrackingBackup() {
  const catalog = await loadCatalog();
  return { format: "ResumeBridgeTrackingBackup", version: 1, exportedAt: new Date().toISOString(),
    catalogSha256: catalog.sourceSha256, ...(await getCatalogState()) };
}

async function importTrackingBackup(payload) {
  const backup = payload.backup;
  const catalog = await loadCatalog();
  if (!isPlainObject(backup) || backup.format !== "ResumeBridgeTrackingBackup" || backup.version !== 1 ||
      backup.catalogSha256 !== catalog.sourceSha256 || !Array.isArray(backup.applications) || !isPlainObject(backup.notes)) {
    throw new Error("备份格式不正确，或备份对应另一份招聘清单。");
  }
  if (backup.applications.length > MAX_JOB_APPLICATIONS) throw new Error("备份记录数超过支持范围。");
  const ids = new Set(catalog.entries.map((e) => e.id));
  if (backup.applications.some((a) => !a || !a.id || !a.companyName || !a.jobTitle || (a.catalogId && !ids.has(a.catalogId)))) {
    throw new Error("备份含无效投递记录，未导入任何内容。");
  }
  const incomingNotes = {};
  for (const [id, note] of Object.entries(backup.notes)) {
    if (!ids.has(id)) throw new Error("备份含未知招聘条目，未导入任何内容。");
    incomingNotes[id] = normalizeCatalogNote(note);
  }
  const existing = await getCatalogState();
  const records = new Map(existing.applications.map((a) => [a.id, a]));
  for (const a of normalizeJobApplications(backup.applications)) {
    if (!records.has(a.id) || a.updatedAt > records.get(a.id).updatedAt) records.set(a.id, a);
  }
  if (records.size > MAX_JOB_APPLICATIONS) throw new Error("合并后的记录数超过支持范围。");
  const notes = { ...existing.notes };
  for (const [id, note] of Object.entries(incomingNotes)) {
    if (!notes[id] || note.updatedAt > notes[id].updatedAt) notes[id] = note;
  }
  await chrome.storage.local.set({ jobApplications: Array.from(records.values()), catalogNotes: notes });
  return { applicationCount: records.size, noteCount: Object.keys(notes).length };
}

async function saveJobApplication(payload) {
  const source = isPlainObject(payload.application) ? payload.application : {};
  if (source.catalogId) await requireCatalogEntry(source.catalogId);
  const result = await chrome.storage.local.get([STORAGE_KEYS.jobApplications]);
  ...
  if (applications.length >= MAX_JOB_APPLICATIONS) throw new Error("投递记录已达上限，请先导出备份并整理旧记录。");
  const normalized = normalizeJobApplications([application, ...applications]);
  await chrome.storage.local.set({ [STORAGE_KEYS.jobApplications]: normalized });
  return { application, created: true, duplicate: false };
}

async function importJobApplications(payload) {
  if (Array.isArray(payload.applications) && payload.applications.length > MAX_JOB_APPLICATIONS) {
    throw new Error("导入记录超过上限，未导入任何内容。");
  }
