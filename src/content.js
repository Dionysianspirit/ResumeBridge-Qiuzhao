(() => {
  ...
  function getControlCurrentValue(element) {
    ...
    if (element.getAttribute?.("role") === "combobox") {
      return normalizeText(element.getAttribute("data-value") ||
        element.getAttribute("aria-valuetext") || getElementText(element), 260);
    }
  ...
  async function expandKnownProfileRepeatersForScan() {
    ...
    for (const config of profileUtils.getRepeatConfigs()) {
      ...
      for (let attempt = 0; attempt < config.maxItems && expanded < MAX_KNOWN_REPEAT_EXPANSIONS; attempt += 1) {
        ...
        // A heading and a generic "添加" button alone do not prove that this is
        // an editable repeater. Leave empty, ambiguous sections for manual use.
        const explicitAction = profileUtils.getConfigForActionText(getButtonText(action.control), "");
        if (currentCount === 0 && explicitAction?.sectionKey !== config.sectionKey) {
          break;
        }
