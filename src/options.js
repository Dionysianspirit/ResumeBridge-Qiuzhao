...
const variantSelect = document.getElementById("profileVariantSelect");
const variantName = document.getElementById("profileVariantName");
const variantCreate = document.getElementById("createProfileVariant");
const variantFeedback = document.getElementById("variantFeedback");

variantSelect.addEventListener("change", () => void changeProfileVariant());
variantCreate.addEventListener("click", () => void saveProfileVariant());
...
async function loadSettings() {
  try {
    ...
    renderProfileVariants(settings);
...
function renderProfileVariants(settings) {
  activeProfileId = settings.activeProfileId;
  profileRevision = settings.profileRevision;
  variantSelect.replaceChildren();
  for (const profile of settings.profileLibrary.profiles) {
    const option = document.createElement("option");
    option.value = profile.id;
    option.textContent = profile.name;
    variantSelect.appendChild(option);
  }
  variantSelect.value = activeProfileId;
  variantSelect.disabled = false;
  variantCreate.disabled = false;
  variantFeedback.textContent = "先编辑资料，再另存为不同岗位的版本；切换简历会影响下一次填写。备份导出仅包含当前版本。";
}

function profileEditContext() {
  return { expectedActiveProfileId: activeProfileId, expectedProfileRevision: profileRevision };
}

async function refreshProfileVariantEditor() {
  const settings = await sendRuntimeMessage({ type: "OJAF_GET_SETTINGS" });
  renderProfileVariants(settings);
  renderProfileSectionEditor(getProfileV2FromSettings(settings));
  setProfileSaved(`当前简历：${variantSelect.selectedOptions[0].textContent}`);
  scheduleProfileSectionSync();
}

async function changeProfileVariant() {
  if (profileHasUnsavedChanges) {
    variantSelect.value = activeProfileId;
    variantFeedback.textContent = "当前资料有未保存修改，请先保存、导出或另存为新简历，再切换。";
    return;
  }
  const targetId = variantSelect.value;
  variantSelect.disabled = variantCreate.disabled = true;
  try {
    await sendRuntimeMessage({ type: "OJAF_SWITCH_PROFILE", payload: { id: targetId, ...profileEditContext() } });
    await refreshProfileVariantEditor();
    showToast("已切换简历。");
  } catch (error) {
    variantSelect.value = activeProfileId;
    variantFeedback.textContent = error.message;
  } finally {
    variantSelect.disabled = variantCreate.disabled = false;
  }
}

async function saveProfileVariant() {
  variantSelect.disabled = variantCreate.disabled = true;
  try {
    await sendRuntimeMessage({
      type: "OJAF_CREATE_PROFILE",
      payload: { name: variantName.value, profileV2: collectProfileV2FromEditor(), ...profileEditContext() }
    });
    variantName.value = "";
    await refreshProfileVariantEditor();
    showToast("新简历已保存并设为当前版本。");
  } catch (error) {
    variantFeedback.textContent = error.message;
  } finally {
    variantSelect.disabled = variantCreate.disabled = false;
...
async function saveProfile() {
  try {
    ...
    const saved = await sendRuntimeMessage({
      type: "OJAF_SAVE_SETTINGS",
      payload: { profileV2, ...profileEditContext() }
    });
...
async function importProfileFromFile() {
  ...
  try {
    ...
    if (!window.confirm("导入会替换当前版本的简历资料。继续前可取消并导出备份。是否继续？")) return;
    const saved = await sendRuntimeMessage({
      type: "OJAF_SAVE_SETTINGS",
      payload: { profileV2, ...profileEditContext() }
    });
...
async function clearLocalData() {
  const confirmed = window.confirm("这会清空本机保存的所有简历版本、填写策略和 API 设置，只影响当前浏览器。是否继续？");
  if (!confirmed) {
    return;
  }
